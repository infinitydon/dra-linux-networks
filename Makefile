IMAGE ?= ghcr.io/infinitydon/dra-linux-networks:0.5.0
HOST_DEVICE_NODE ?= ebpf-bng-node-02
STATIC_ADDRESS ?= 192.168.88.10/24
GATEWAY ?= 192.168.88.1

.PHONY: test test-e2e-multi-node test-e2e-repeated-network test-e2e-host-device test-e2e-dpdk test-e2e-vpp test-e2e-vpp-multi build image
...
test-e2e-repeated-network:
	go test -tags=e2e ./tests/e2e -v -run TestRepeatedTemplateAllocatesUniquePodInterfaceNames -args -dynamic-node "$(DYNAMIC_NODE)"
